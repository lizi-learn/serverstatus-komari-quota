...
- `/relays` 在同一站点内提供中转订阅、复制按钮、卡片/列表切换和秒级实时状态；CPU 或内存高负载闪红，高速传输闪蓝，两者同时出现时紫色间歇闪烁，并遵循系统的“减少动态效果”设置。网速和月流量沿用 Komari 整机口径，不会伪装成逐协议统计；隧道流量可能同时出现在物理和虚拟接口上。
...
在 Komari 后台把 `traffic.up` 和 `traffic.down` 的指标保留时间设为至少 35 天，再于主题管理页面上传仓库根目录的 `ServerStatusQuota4-v1.0.18-quota.13.zip` 并应用。要在中转页显示节点，为节点标签增加 `relay=sing-box`。ZIP 根目录包含：
