...
在 Komari 后台把 `traffic.up` 和 `traffic.down` 的指标保留时间设为至少 35 天，再于主题管理页面上传仓库根目录的 `ServerStatusQuota4-v1.0.18-quota.16.zip` 并应用。要在首页中转控制条和流量高亮中包含节点，为节点标签增加 `relay=sing-box`。ZIP 根目录包含：
