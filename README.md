...
- 中转订阅、复制按钮、在线数和实时总速率以透明紧凑控制条合并在首页表格下方；原表格中明确带有 `relay=sing-box` 标签的节点会按整机实时上下行显示蓝色传输高亮，不再创建重复卡片，也不以 CPU/RAM 触发动画。旧 `/relays` 地址会自动回到首页。网速沿用 Komari 整机口径，不会伪装成逐协议统计；隧道流量可能同时出现在物理和虚拟接口上。
...
在 Komari 后台把 `traffic.up` 和 `traffic.down` 的指标保留时间设为至少 35 天，再于主题管理页面上传仓库根目录的 `ServerStatusQuota4-v1.0.18-quota.15.zip` 并应用。要在首页中转控制条和流量高亮中包含节点，为节点标签增加 `relay=sing-box`。ZIP 根目录包含：
