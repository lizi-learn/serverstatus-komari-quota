...
- 中转订阅、复制按钮、在线数和实时下载/上传总速率以透明紧凑控制条合并在首页表格下方；首页所有在线节点都会按整机实时上下行显示蓝色传输高亮：超过 128 KiB/s 是柔和呼吸，超过 1 MiB/s 是更明显的快速动效。中转控制条仍只统计明确带有 `relay=sing-box` 标签的节点，不创建重复卡片，也不以 CPU/RAM 触发流量动画。首页负载列隐藏，展开节点详情仍可查看。旧 `/relays` 地址会自动回到首页。网速沿用 Komari 整机口径，不会伪装成逐协议统计；隧道流量可能同时出现在物理和虚拟接口上。
